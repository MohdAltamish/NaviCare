...
function BrowsePageContent() {
  ...
  const [selectedCategoryState, setSelectedCategoryState] = React.useState<string | null>(null);

  const selectedCategory = selectedCategoryState !== null ? selectedCategoryState : urlCategory;
                  ...
                  {CATEGORIES.map((cat) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left rounded-lg px-3 py-2 text-sm font-medium cursor-pointer transition-all"
                        style={{
                          backgroundColor: isActive
                            ? "var(--nc-green)"
                            : "transparent",
                          color: isActive ? "#FFFFFF" : "var(--nc-body)",
                        }}
                        onMouseEnter={(e) => {
                          if (!isActive) {
              ...
              <div className="space-y-4">
                {filteredPrograms.map((program) => {
                  ...
                  return (
                    <div
                      key={program.slug}
                      className="rounded-xl p-6 bg-white transition-all"
                      style={{
                        border: "1px solid var(--nc-card-border)",
                        borderLeft: `4px solid ${cat?.border || "var(--nc-green)"}`,
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.borderColor = cat?.border || "var(--nc-green)";
                        e.currentTarget.style.borderLeftColor = cat?.border || "var(--nc-green)";
                      }}
                      onMouseLeave={(e) => {
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                        <span
                          ...
                          style={{
                            backgroundColor: `${cat?.border || "var(--nc-green)"}10`,
                            color: cat?.border || "var(--nc-green)",
                          }}
                        >
                          {categoryColors[program.category as BenefitCategory]?.emoji}{" "}
                          {cat?.label}
                        </span>
                        <span
                      ...
                      {program.eligibility.income_limits.length > 0 && (
                        ...
                        >
                          ...
                          {program.eligibility.income_limits[
                            Math.min(
                              2,
                              program.eligibility.income_limits.length - 1
                            )
    ...
    <Suspense fallback={
      <>
        ...
        <main className="nc-container flex flex-col items-center justify-center text-center" style={{ minHeight: "60vh", paddingTop: "80px" }}>
          ...
          <p className="text-base font-medium" style={{ color: "var(--nc-body)" }}>Loading programs...</p>
        </main>
      </>
    }>
      <BrowsePageContent />
